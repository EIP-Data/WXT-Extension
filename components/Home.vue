...
<template>
  <div class="p-6 space-y-6 bg-gray-50 dark:bg-gray-900">
    <!-- Header Section -->
    <div class="space-y-1">
      <h1 class="text-xl font-bold text-gray-900 dark:text-gray-50">
        {{ t("home.welcome") }},
        <span class="text-orangePrimary dark:text-orangePrimary-dark">{{ user || t("home.user") }}</span>
      </h1>
    ...
    <div class="grid grid-cols-1 gap-3">
      <!-- Cookies Card -->
      <div class="bg-orangePrimary/10 dark:bg-orangePrimary-dark/20 p-4 rounded-xl transition-all duration-300 hover:scale-[1.02]">
        <div class="flex items-center gap-3">
          <div class="p-2.5 bg-orangePrimary/20 dark:bg-orangePrimary-dark/30 rounded-lg">
            <svg class="w-7 h-7 text-orangePrimary dark:text-orangePrimary-dark" viewBox="0 0 32 32">
          ...
          <div class="flex-1">
            ...
            <p class="text-gray-600 dark:text-gray-300 text-xs">{{ t("home.cookies_blocked") }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-orangePrimary/10 dark:bg-orangePrimary-dark/20 p-4 rounded-xl transition-all duration-300 hover:scale-[1.02]">
        <div class="flex items-center gap-3">
          <div class="p-2.5 bg-orangePrimary/20 dark:bg-orangePrimary-dark/30 rounded-lg">
            <ShieldCheckIcon class="w-7 h-7 text-orangePrimary dark:text-orangePrimary-dark" />
          </div>
          <div class="flex-1">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.trackersStopped }}</h3>
            <p class="text-gray-600 dark:text-gray-300 text-xs">{{ t("home.trackers_stopped") }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-orangePrimary/10 dark:bg-orangePrimary-dark/20 p-4 rounded-xl transition-all duration-300 hover:scale-[1.02]">
        <div class="flex items-center gap-3">
          <div class="p-2.5 bg-orangePrimary/20 dark:bg-orangePrimary-dark/30 rounded-lg">
            <XMarkIcon class="w-7 h-7 text-orangePrimary dark:text-orangePrimary-dark" />
          </div>
          <div class="flex-1">
            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ stats.adsPrevented }}</h3>
            <p class="text-gray-600 dark:text-gray-300 text-xs">{{ t("home.ads_prevented") }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <ProfilingChart
          ...
          :metrics="{
          trackers: [
            { type: 'Analytics', count: 45, percentage: 37.5 },
            { type: 'Ads', count: 32, percentage: 26.7 },
            { type: 'Social', count: 28, percentage: 23.3 },
            { type: 'Other', count: 15, percentage: 12.5 }
          ],
          ads: [],
          locations: []
        }"
